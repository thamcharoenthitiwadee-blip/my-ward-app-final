    ...
    try {
      ...
      if (leaveType) {
        ...
        for (const [id, data] of activeShifts) {
          ...
          names.push(sThai);
          types.push(data.workType);
          
          const OT_RATE = 650 / 8;
          let currentPrice = 0;
          if (data.workType === 'NORMAL') {
            currentPrice = (id === 'morn' ? 0 : 360) + (data.extraHours * OT_RATE);
            totalHours += data.extraHours;
          } else {
            ...
            currentPrice = data.workType.startsWith('REF') ? (ref?.price || 0) : (data.hours * OT_RATE);
            totalHours += data.workType.startsWith('REF') ? 0 : data.hours;
          }
          totalPrice += currentPrice;
        }
        finalShiftName = names.join("/");
        finalWorkType = Array.from(new Set(types)).join("/");
      }

      const payload = { date: selectedDate, nurseName, shiftName: finalShiftName, workType: finalWorkType, hours: totalHours, total: totalPrice };
        ...
        {view === 'RECORD' ? (
          ...
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden border">
            <div className="bg-indigo-700 p-6 text-white flex justify-between items-center">
              <h2 className="text-xl font-bold uppercase tracking-widest text-white">ตารางปฏิบัติงานนรีเวช</h2>
              <button onClick={fetchData} className="text-xs bg-indigo-600 px-4 py-2 rounded-full border border-indigo-400 hover:bg-indigo-500 transition-all text-white">รีเฟรช</button>
            </div>
                          ...
                          <td key={i} className="border p-1 text-center h-10 bg-white">
                            <div className="flex flex-row items-center justify-center gap-0.5">
                              {dRecs.map((record, index) => {
                                // แยกข้อมูลที่อาจจะมีเครื่องหมาย "/" เชื่อมกัน
                                ...
                                return (
                                  <div key={index} className="flex gap-0.5">
                                    {sArr.map((s: string, sIdx: number) => {
                                      ...
                                      const type = tArr[sIdx] || tArr[0]; // เลือกประเภทงานให้ตรงกับเวร หรือใช้ค่าแรกสุดถ้ามีค่าเดียว
                                      ...
                                      return (
                                        <span key={sIdx} className="inline-flex items-start">
                                          ...
                                          {/* ✨ ส่วนปรับปรุงใหม่: แสดงคำว่า OT หรือ BB ตามข้อมูลจริงแบบตัวยก ✨ */}

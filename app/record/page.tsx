...
export default function WardShiftApp() {
  ...
  const [nurses] = useState(["คุณสมศรี ใจดี", "คุณสมชาย แข็งแรง", "คุณวิภา ขยันทำงาน", "คุณตะวัน"]);
  const [selectedNurse, setSelectedNurse] = useState(nurses[0]);
  ...
  const nurseSummaries = useMemo(() => {
    ...
    sheetData.forEach(row => {
      ...
      if (!summary[name]) {
        summary[name] = { shiftBase: 0, otAndOthers: 0 };
      }

      // แยก "ค่าเวร" ออกจาก "เงินอื่นๆ"
      if (type === 'NORMAL') {
        const baseWage = (shift === 'เช้า' ? 0 : 360);
        summary[name].shiftBase += baseWage;
        summary[name].otAndOthers += (totalAmount - baseWage); // ส่วนที่เหลือคือค่าล่วงเวลา
      } else {
        // ถ้าเป็น OT หรือ Refer ทั้งหมดถือว่าเป็น "เงินอื่นๆ"
  ...
  const handleSaveToSheet = async () => {
    ...
    for (const [id, data] of activeShifts) {
      const payload = {
        date: selectedDate, nurseName: selectedNurse, 
        ...
        {view === 'RECORD' ? (
          ...
          <div className="max-w-md mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden border p-5 space-y-5">
            <div className="grid grid-cols-2 gap-3 bg-slate-50 p-4 rounded-2xl border">
              <select value={selectedNurse} onChange={(e) => setSelectedNurse(e.target.value)} className="w-full p-2 bg-white border rounded-lg text-xs font-bold text-green-700 outline-none">{nurses.map(name => <option key={name} value={name}>{name}</option>)}</select>
              <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full p-2 bg-white border rounded-lg text-xs font-bold outline-none" />
            </div>
              ...
              {(['morn', 'aft', 'night'] as const).map((id) => (
                <div key={id} className={`p-4 rounded-2xl border-2 transition-all ${shifts[id].active ? 'border-green-500 bg-white shadow-md' : 'border-slate-100 bg-slate-50 opacity-50'}`}>
                   <div className="flex items-center gap-3 mb-2">
                    <input type="checkbox" checked={shifts[id].active} onChange={() => setShifts({...shifts, [id]: {...shifts[id], active: !shifts[id].active}})} className="w-6 h-6 rounded text-green-600" />
                    ...
                    <div className="space-y-3 mt-2 border-t pt-3">
                      <div className="flex flex-wrap gap-1">
                        {[{id:'NORMAL', label:'NORMAL'},{id:'OT', label:'OT'},{id:'BB', label:'BB'},{id:'UNIT', label:'ออกหน่วย'},{id:'CT', label:'CT'},{id:'OPD', label:'OPD'},{id:'REF_NO', label:'Ref(ไม่มี)'},{id:'REF_WITH', label:'Ref(มี)'},{id:'REF_OUT', label:'Ref(นอก)'},{id:'REF_BACK', label:'Ref(Back)'}].map((type) => (
                          <button key={type.id} onClick={() => setShifts({...shifts, [id]: {...shifts[id], workType: type.id}})}
                            className={`px-2 py-1 rounded-md text-[9px] font-bold border-2 ${shifts[id].workType === type.id ? 'bg-green-600 text-white' : 'bg-white text-slate-400 border-slate-100'}`}
                          >{type.label}</button>
                        ))}
                      </div>
                      {shifts[id].workType === 'NORMAL' && (
                        <div className="flex items-center gap-2 bg-blue-50 p-2 rounded-xl border border-blue-100">
                          ...
                          <input type="number" value={shifts[id].extraHours} onChange={(e) => setShifts({...shifts, [id]: {...shifts[id], extraHours: Number(e.target.value)}})} className="w-16 p-1 bg-white border rounded text-center font-bold text-blue-700" />
                        </div>
                      )}
                      {!['NORMAL'].includes(shifts[id].workType) && !shifts[id].workType.startsWith('REF') && (
                        <div className="flex items-center gap-2 bg-amber-50 p-2 rounded-xl border border-amber-100">
                          ...
                          <input type="number" value={shifts[id].hours} onChange={(e) => setShifts({...shifts, [id]: {...shifts[id], hours: Number(e.target.value)}})} className="w-16 p-1 bg-white border rounded text-center font-bold text-amber-700" />
                        </div>
                      )}
                    </div>
                  )}
            ...
            <button onClick={handleSaveToSheet} disabled={isSaving} className="w-full bg-green-600 text-white py-5 rounded-2xl font-black text-xl shadow-lg uppercase">{isSaving ? "Saving..." : "บันทึกลง Sheets"}</button>
          </div>
        ) : (
          /* หน้า Dashboard สรุปยอดรายคนตามที่คุณขอ */
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-200">
            <div className="bg-indigo-700 p-6 text-white flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold uppercase tracking-widest">สรุปยอดรายรายบุคคล</h2>
                <p className="text-indigo-200 text-[10px] mt-1 italic">คำนวณแยกค่าเวร และ OT/งานพิเศษอื่นๆ</p>
              </div>
              <button onClick={fetchDashboardData} className="text-xs bg-indigo-600 px-4 py-2 rounded-full border border-indigo-400">รีเฟรชข้อมูล</button>
            </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    ...
                    <th className="p-4 text-right">รวมค่าเวร (360.-)</th>
                    <th className="p-4 text-right">รวม OT & อื่นๆ</th>
                    <th className="p-4 text-right text-indigo-600 font-black">ยอดรวมสุทธิ</th>
                  </tr>
                </thead>
                <tbody className="text-sm">
                  {nurseSummaries.length > 0 ? nurseSummaries.map(([name, data], i) => (
                    <tr key={i} className="border-b hover:bg-slate-50 transition-colors">
                      <td className="p-4 font-black text-slate-700 text-lg">{name}</td>
                      <td className="p-4 text-right font-medium text-emerald-600">{data.shiftBase.toLocaleString()}</td>
                      <td className="p-4 text-right font-medium text-orange-500">{data.otAndOthers.toLocaleString()}</td>
                      <td className="p-4 text-right font-black text-xl text-indigo-700">{(data.shiftBase + data.otAndOthers).toLocaleString()}</td>
                    </tr>
                  )) : (
                    <tr><td colSpan={4} className="p-10 text-center text-slate-300 italic">ยังไม่มีข้อมูลสำหรับสรุปยอด...</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            <div className="p-6 bg-slate-50 border-t">
              <p className="text-[10px] text-slate-400 leading-relaxed">* ระบบจะนำยอดเงินรวมในแต่ละบรรทัดมาลบด้วยค่าเวรพื้นฐาน (360 บาท สำหรับบ่าย/ดึก) เพื่อหาผลรวมของ OT, ค่าล่วงเวลา, Refer และงานพิเศษอื่นๆ ทั้งหมดให้โดยอัตโนมัติ</p>
            </div>
          </div>
        )}
      </div>
